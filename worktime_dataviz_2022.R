...
# Get data
timetrack_df <- read_csv("/Users/simon/Dropbox/1_orga/Timetracking/Clockify_Time_Report_Detailed_01_01_2022-12_31_2022.csv")
timetrack_old_df <- read_csv("/Users/simon/Dropbox/1_orga/Timetracking/Clockify_Time_Report_Detailed_01_01_2021-12_31_2021.csv")
...
start <- seq(as.POSIXct("2022-01-01 0:00:00", tz = "UTC"), as.POSIXct("2022-12-31 23:00:00", tz="UTC"), by="hour")
end <- seq(as.POSIXct("2022-01-01 1:00:00", tz = "UTC"), as.POSIXct("2023-01-01 00:00:00", tz="UTC"), by="hour")
...
p_day <- ggplot(data=spells_df, aes(x=start_hour_int, y=minutes_worked)) +
  ...
  annotate("text", x = 17.95, y = c(11420, 9100), label = c("A lunch-break in", "the distribution"),family="xkcd",colour="red") +
...
task_area_hours_old <- task_area_hours_old %>% mutate(
  ...
  select(task_area, change)

# Bar chart for change
p_change <- ggplot(data=task_area_hours_old) +
  geom_col(aes(change, task_area, fill = task_area)) +
  theme_xkcd +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        legend.position = "none",
        legend.title = element_blank()) +
  xlab("") + ylab("") + ggtitle("Compared to last year") + 
  scale_x_continuous(
    breaks = c(-50, 50),
...
p_man <- p + xkcdman(mapping,dataman) + theme_xkcd() +
  ...
  annotate("text", x=100, y = -800,
           label = "I somehow spent even \n more time on admin tasks \n  compared to last year!", family="xkcd" ) +
...
ggsave("timetrack2022.jpg", final, width=2900, height = 2000, units = "px")
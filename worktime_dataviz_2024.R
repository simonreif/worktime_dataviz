...
# Get data
timetrack_df <- read_csv("OrigData/Clockify_Time_Report_Detailed_01_01_2024-12_31_2024.csv")
timetrack_old_df <- read_csv("OrigData/Clockify_Time_Report_Detailed_01_01_2023-12_31_2023.csv")
...
start <- seq(as.POSIXct("2024-01-01 0:00:00", tz = "UTC"), as.POSIXct("2024-12-31 23:00:00", tz = "UTC"), by = "hour")
end <- seq(as.POSIXct("2024-01-01 1:00:00", tz = "UTC"), as.POSIXct("2025-01-01 00:00:00", tz = "UTC"), by = "hour")
...
p_day <- ggplot(data = spells_df, aes(x = start_hour_int, y = minutes_worked)) +
  ...
  annotate("text", x = 17.95, y = c(11420, 9100), label = c("A lunch-break in", "the distribution"),
           family = "xkcd Script", colour = "red") +
  ggtitle("By hour of the day") +
  geom_curve(aes(x = x1, y = y1, xend = x2, yend = y2, group = z),
    data = dff,
    colour = "red",
    size = 0.8,
    curvature = +0.2,
...
task_area_hours_old <- task_area_hours_old %>%
  mutate(
...
p_change <- ggplot(data = task_area_hours_old) +
  ...
  xlab("") +
  ylab("") +
  ggtitle("Shares compared to last year") +
  scale_x_continuous(
    breaks = c(-0.1, 0.1),
...
dow_hours <- timetrack_mod %>%
  mutate(
    dow = ifelse(dow == 1, 8, dow),
    dow = dow - 1
  ) %>%
  group_by(dow) %>%
  summarise(
    hours = sum(`Duration (decimal)`),
...
p <- ggplot(data = d, aes(x = x, y = y)) +
  geom_point(colour = "white", alpha = 0.5, size = 5)
...
mapping <- aes(
  x = y, y = y, scale, ratioxy, angleofspine,
...
p_man <- p + xkcdman(mapping, dataman) + theme_xkcd() +
  ...
  annotate("text",
    x = 100, y = -800,
    label = "Yikes, less than  \n 50% admin work!", family = "xkcd Script"
  ) +
...
ggsave("timetrack2024.jpg", final, width = 2900, height = 2000, units = "px")

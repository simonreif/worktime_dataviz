...
# Get data
timetrack_2021 <- read_csv("/Users/simon/Dropbox/1_orga/Timetracking/Clockify_Time_Report_Detailed_01_01_2021-12_31_2021.csv")

# Clean Data
timetrack_2021_mod <- timetrack_2021 %>%
...
start <- seq(as.POSIXct("2021-01-01 0:00:00", tz = "UTC"), as.POSIXct("2021-12-31 23:00:00", tz="UTC"), by="hour")
end <- seq(as.POSIXct("2021-01-01 1:00:00", tz = "UTC"), as.POSIXct("2022-01-01 00:00:00", tz="UTC"), by="hour")
df_times <- data.table(start, end)
setkey(df_times, start, end)

# Make data table with actual start-end times
timetrack_hours_dt <- data.table(timetrack_2021_mod %>%
...
#### Time per Task Area ####
task_area_hours <- timetrack_2021_mod %>%
  group_by(task_area) %>%
...
#### Work time per Month ####
month_hours <- timetrack_2021_mod %>%
...
ggsave("timetrack2021.jpg", final, width=2600, height = 1480, units = "px")